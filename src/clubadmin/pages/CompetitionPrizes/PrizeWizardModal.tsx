...
type SpotPrizeSource = "early_bird" | "all_competitors";
type WinLimitMode = "once_per_scope" | "unlimited";
...
export type PrizeWizardState = {
    ...
    disciplineFishTypeId: string | null;
    ...
    // ✅ NEW — Spot prize list generation
    spotSource: SpotPrizeSource | null;   // early bird vs all competitors
    winLimit: WinLimitMode;               // once vs unlimited
...
export function defaultPrizeWizardState(opts: {
    ...
    export default function PrizeWizardModal({
        ...
        // -------------------------------------------------------------------------
        // TEMP DEBUG — Prize wizard state snapshots (FULL + INTENTIONAL)
        // -------------------------------------------------------------------------

        const canProceedDebug = (() => {
            try {
                return canNext();
            } catch {
                return false;
        ...
        if (wiz.prizeType === "species") {
            console.log("[PrizeWizard] Species prize snapshot", {
                ...
                selectedKeyCount: wiz.selectedKeys.length,

                selectedSpeciesKeys: wiz.selectedKeys.filter(k => k.startsWith("s:")),
                selectedGroupKeys: wiz.selectedKeys.filter(k => k.startsWith("g:")),
                ...
                resolvedResultMethod:
                    wiz.award_rule === "ranked"
                        ? (context.allowedResultMethods.length === 1
                            ? context.allowedResultMethods[0]
                            : wiz.result_method)
                        : null,
                ...
                // Validation hints
                validation: {
                    hasPrizeType: !!wiz.prizeType,
                    disciplineRequired:
                        wiz.prizeType === "species" &&
                        context.discipline === "mixed" &&
                        safeFishTypes.length > 1,
                    hasDiscipline: !!wiz.disciplineFishTypeId,
                    hasTargets: wiz.selectedKeys.length > 0,
                    hasAwardRule: !!wiz.award_rule,
                    hasOutcomes:
        ...
        if (wiz.prizeType === "special") {
            console.log("[PrizeWizard] Special / Spot prize snapshot", {
                ...
                // Core spot-prize fields
                specialName: wiz.specialName,
                specialNameValid: wiz.specialName.trim().length > 0,

                specialDescription: wiz.specialDescription,

                spotSource: wiz.spotSource,
                winLimit: wiz.winLimit,

                // Award config (forced internally, but logged for sanity)
                award_rule: wiz.award_rule,
                count: wiz.count,
                outcomes: wiz.outcomes,
                result_method: wiz.result_method,

                // Targets (should always be empty for special)
                targetMode: wiz.targetMode,
                selectedKeys: wiz.selectedKeys,

                // Derived / enforced behaviour
                enforcedRules: {
                    alwaysSinglePrize: true,
                    awardRuleForcedToFirst: true,
                    resultMethodIgnored: true,
                    outcomesIgnored: true,
                },

                // Validation hints (THIS is what was blocking you)
                validation: {
                    hasName: wiz.specialName.trim().length > 0,
                    hasSpotSource: !!wiz.spotSource,
                    hasWinLimit: !!wiz.winLimit,
                    shouldBeAbleToProceed:
                        wiz.specialName.trim().length > 0 &&
                        !!wiz.spotSource &&
                        !!wiz.winLimit,
                },
            });
                            ...
                            {wiz.step === 1 && (
                                <div style={{ display: "grid", gap: 12 }}>
                                    ...
                                    {wiz.prizeType === "special" && (
                                        <>
                                            {/* ───────────────────────────────────────────── */}
                                            {/* SPOT PRIZE NAME (REQUIRED) */}
                                            {/* ───────────────────────────────────────────── */}
                                            <div className="wizard-section">
                                                <div className="wizard-question-title">
                                                    What is this prize called?
                                                </div>

                                                <input
                                                    className="input"
                                                    placeholder="e.g. Early Bird Spot Prize"
                                                ...
                                                {!wiz.specialName.trim() && (
                                                    <div className="wizard-help">
                                                        A prize name is required to continue.
                                                    </div>
                                                )}
                                            </div>

                                            {/* ───────────────────────────────────────────── */}
                                            {/* SPOT PRIZE DESCRIPTION (OPTIONAL) */}
                                            {/* ───────────────────────────────────────────── */}
                                            <div className="wizard-section">
                                                <div className="wizard-question-title">
                                                ...
                                                <textarea
                                                    className="input"
                                                    rows={3}
                                                    placeholder="Random draw at prize giving, judged by committee, etc."
                                            ...
                                            <div className="wizard-section">
                                                <div className="wizard-question-title">
                                                    Who is eligible for this draw?
                                                </div>

                                                <div style={{ display: "grid", gap: 10, marginTop: 10 }}>
                                                    <button
                                                        type="button"
                                                        className={`btn btn--ghost ${wiz.spotSource === "early_bird" ? "primary" : ""}`}
                                                        onClick={() =>
                                                            setWiz((w) => ({ ...w, spotSource: "early_bird" }))
                                                        }
                                                        style={{ justifyContent: "flex-start", textAlign: "left" }}
                                                    >
                                                        <div>
                                                            ...
                                                            <div className="muted" style={{ fontSize: 12 }}>
                                                                Only competitors marked as early bird.
                                                            </div>
                                                        </div>
                                                    </button>
                                                    ...
                                                    <button
                                                        type="button"
                                                        className={`btn btn--ghost ${wiz.spotSource === "all_competitors" ? "primary" : ""}`}
                                                        onClick={() =>
                                                            setWiz((w) => ({ ...w, spotSource: "all_competitors" }))
                                                        }
                                                        style={{ justifyContent: "flex-start", textAlign: "left" }}
                                                    >
                                                        <div>
                                                            ...
                                                            <div className="muted" style={{ fontSize: 12 }}>
                                                                Any registered competitor in this scope.
                                                            </div>
                                                        </div>
                                                    </button>
                                                </div>
                                            ...
                                            <div className="wizard-section">
                                                ...
                                                <div style={{ display: "grid", gap: 10, marginTop: 10 }}>
                                                    <button
                                                        type="button"
                                                        className={`btn btn--ghost ${wiz.winLimit === "once_per_scope" ? "primary" : ""}`}
                                                        onClick={() =>
                                                            setWiz((w) => ({ ...w, winLimit: "once_per_scope" }))
                                                        }
                                                        style={{ justifyContent: "flex-start", textAlign: "left" }}
                                                    >
                                                        <div>
                                                            ...
                                                            <div className="muted" style={{ fontSize: 12 }}>
                                                                Once drawn, this person is excluded from future draws
                                                                in this prize scope.
                                                            </div>
                                                        </div>
                                                    </button>
                                                    ...
                                                    <button
                                                        type="button"
                                                        className={`btn btn--ghost ${wiz.winLimit === "unlimited" ? "primary" : ""}`}
                                                        onClick={() =>
                                                            setWiz((w) => ({ ...w, winLimit: "unlimited" }))
                                                        }
                                                        style={{ justifyContent: "flex-start", textAlign: "left" }}
                                                    >
                                                        <div>
                                                            ...
                                                            <div className="muted" style={{ fontSize: 12 }}>
                                                                The same person may be drawn multiple times.
                                                            </div>
                                                        </div>
                                                    </button>
                                                </div>
