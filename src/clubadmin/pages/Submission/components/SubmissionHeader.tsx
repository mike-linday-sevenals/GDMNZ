...
export default function SubmissionHeader({
    ...
    // ------------------------------------------------------------------
    // Keep SubmissionContext in sync with selected competition
    // ------------------------------------------------------------------
    useEffect(() => {
        setCompetitionId(competition?.id ?? null);
    }, [competition?.id, setCompetitionId]);

    // ------------------------------------------------------------------
    // When competition changes, clear fields that shouldn't carry over
    // ------------------------------------------------------------------
    useEffect(() => {
        setDraft(d => ({
            ...d,
            ...
            // optional “carryover guards” (comment out if you want to preserve)
            species_id: null,
            species_name: null,
            outcome: null,
            weight_kg: null,
    ...
    useEffect(() => {
        ...
        (async () => {
            try {
                const full = await getCompetition(organisationId, competition.id);
                if (!cancelled) setFullCompetition(full);
            } catch {
                if (!cancelled) setFullCompetition(null);
            }
        })();
    ...
    useEffect(() => {
        ...
        (async () => {
            ...
            try {
                const rows = await listCompetitorsForCompetition(competition.id);
    ...
    return (
        <>
            <section className="card" style={{ width: "100%" }}>
                ...
                >
                    <h2 style={{ margin: 0, flex: "1 1 auto" }}>Catch Submission</h2>

                    <select
                        value={competition?.id ?? ""}
                        onChange={(e) => {
                            const next = competitions?.find((c) => c.id === e.target.value) || null;
                            setCompetition(next);
                    ...
                    <button
                        className={showReview ? "btn primary" : "btn"}
                        onClick={onToggleReview}
                        disabled={!competition?.id}
                        style={{ whiteSpace: "nowrap" }}
                        title="Review and edit submitted fish"
                    >
                ...
                {competition && (
                    ...
                    >
                        {selectedCompetitor ? (
                            <>
                                <div>
                                    <div style={{ fontSize: 12, color: "#64748b" }}>Angler</div>
                                    <div style={{ fontWeight: 600 }}>{selectedCompetitor.full_name}</div>
                                </div>

                                <div>
                                    <div style={{ fontSize: 12, color: "#64748b" }}>Boat</div>
                                    <div style={{ fontWeight: 600 }}>{selectedCompetitor.boat}</div>
                                </div>
                                ...
                                <div>
                                    <div style={{ fontSize: 12, color: "#64748b" }}>Skipper</div>
                                    <input
                                        type="text"
                ...
                {competition && draft.competitor_id && (
                    <>
                        ...
                        >
                            <div>
                                <div style={{ fontSize: 12, color: "#64748b" }}>Catch date</div>
                                ...
                                {draft.date_caught &&
                                    ...
                                    !competitionDays.some((d) => d.day_date === draft.date_caught) && (
                            ...
                            <div>
                                <div style={{ fontSize: 12, color: "#64748b" }}>Hooked time</div>
                                <input
                                    type="time"
                            ...
                            <div>
                                <div style={{ fontSize: 12, color: "#64748b" }}>Landed time</div>
                                <input
                                    type="time"
                        ...
                        >
                            ...
                            {!draft.is_courtesy_weigh && (
                                <div>
                                    <div style={{ fontSize: 12, color: "#64748b" }}>Club number</div>
                                    <input
                                        type="text"
                                        placeholder="Club #"
                            ...
                            {draft.is_courtesy_weigh && (
                                <div style={{ gridColumn: "2 / span 2" }}>
                                    <div style={{ fontSize: 12, color: "#64748b" }}>Visiting club</div>
                                    <input
                                        type="text"
                ...
                {competition && draft.competitor_id && compType === "mixed" && (
                    <div style={{ marginTop: 20, textAlign: "center" }}>
                        ...
                        <div style={{ display: "flex", gap: 10, justifyContent: "center" }}>
                            <button
                                className={discipline === "sport" ? "btn primary" : "btn"}
                            ...
                            <button
                                className={discipline === "game" ? "btn primary" : "btn"}
            ...
            {competition && draft.competitor_id && discipline === "sport" && <SportCatchCard />}
        </>
    );
